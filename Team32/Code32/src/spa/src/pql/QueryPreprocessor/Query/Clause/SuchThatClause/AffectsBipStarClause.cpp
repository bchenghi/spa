#include "AffectsBipStarClause.h"
#include <set>

using pql::AffectsBipStarClause;
using pql::FilterResult;
using pql::PkbAbstractor;

AffectsBipStarClause::AffectsBipStarClause(QueryArg firstArg, QueryArg secondArg) : SuchThatClause(firstArg, secondArg) {
    if (!SyntaxCheck::isStmtRef(firstArg) || !SyntaxCheck::isStmtRef(secondArg)) {
        throw "Affects Bip Star Clause: arguments do not match the grammar.";
    }
...
FilterResult AffectsBipStarClause::executePKBAbsQuery(PkbAbstractor *pkbAbstractor) {
    int stmtNum;
    int stmtNum1;
    ...
    list<pair<StmtNum, StmtNum>> pkbResults = pkbAbstractor->getDataFromAffectsBipStar(stmtNum, stmtNum1);
...
bool AffectsBipStarClause::argIsValid(QueryArg arg) {
