...
NextClause::NextClause(QueryArg firstArg, QueryArg secondArg) : SuchThatClause(firstArg, secondArg) {
    ...
    if (!argIsValid(firstArg) || !argIsValid(secondArg)) {
        if (!pql::SyntaxCheck::isSyntaxCheck()) throw SemanticError("Next Clause: Argument cannot be a variable, constant or procedure");
    }
}
...
FilterResult NextClause::executePKBAbsQuery(PkbAbstractor *pkbAbstractor) {
    ...
    if (firstArg.getQueryArgValue() != nullptr && secondArg.getQueryArgValue() != nullptr) {
        ...
        pkbResults = pkbAbstractor->getNext(stmtNum, stmtNum1);
    } else if (firstArg.getQueryArgValue() != nullptr && secondArg.isWildCardArg()) {
        // NumWildcard
        stmtNum = std::stoi(firstArg.getQueryArgValue()->getValue());
        designEntity1 = DesignEntity::NONE;
        pkbResults = pkbAbstractor->getNext(stmtNum, designEntity1);
        ...
        stmtNum1 = std::stoi(secondArg.getQueryArgValue()->getValue());
        pkbResults = pkbAbstractor->getNext(designEntity, stmtNum1);
    } else if (firstArg.isWildCardArg() && secondArg.isWildCardArg()) {
        // WildcardWildcard
        designEntity = DesignEntity::NONE;
        designEntity1 = DesignEntity::NONE;
        pkbResults = pkbAbstractor->getNext(designEntity, designEntity1);
...
bool NextClause::argIsValid(QueryArg arg) {
