#include "NextBipStarClause.h"
#include <set>

using std::set;
using pql::FilterResult;
using pql::NextBipStarClause;

NextBipStarClause::NextBipStarClause(QueryArg firstArg, QueryArg secondArg) : SuchThatClause(firstArg, secondArg) {
    if (!SyntaxCheck::isStmtRef(firstArg) || !SyntaxCheck::isStmtRef(secondArg)) {
        throw "Next Bip Star Clause: arguments do not match the grammar.";
    }

    if (!argIsValid(firstArg) || !argIsValid(secondArg)) {
        if (!pql::SyntaxCheck::isSyntaxCheck()) throw SemanticError("Next Bip Star Clause: Argument cannot be a variable, constant or procedure");
    }
}

FilterResult NextBipStarClause::executePKBAbsQuery(PkbAbstractor *pkbAbstractor) {
    int stmtNum;
    ...
    if (firstArg.getQueryArgValue() != nullptr && secondArg.getQueryArgValue() != nullptr) {
        ...
        pkbResults = pkbAbstractor->getNextBipStar(stmtNum, stmtNum1);
    } else if (firstArg.getQueryArgValue() != nullptr && secondArg.isWildCardArg()) {
        // NumWildcard
        stmtNum = std::stoi(firstArg.getQueryArgValue()->getValue());
        designEntity1 = DesignEntity::NONE;
        pkbResults = pkbAbstractor->getNextBipStar(stmtNum, designEntity1);
        ...
        stmtNum1 = std::stoi(secondArg.getQueryArgValue()->getValue());
        pkbResults = pkbAbstractor->getNextBipStar(designEntity, stmtNum1);
    } else if (firstArg.isWildCardArg() && secondArg.isWildCardArg()) {
        // WildcardWildcard
        designEntity = DesignEntity::NONE;
        designEntity1 = DesignEntity::NONE;
        pkbResults = pkbAbstractor->getNextBipStar(designEntity, designEntity1);
...
bool NextBipStarClause::argIsValid(QueryArg arg) {
