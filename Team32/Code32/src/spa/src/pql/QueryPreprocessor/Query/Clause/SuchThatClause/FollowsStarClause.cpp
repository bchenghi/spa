...
FollowsStarClause::FollowsStarClause(QueryArg firstArg, QueryArg secondArg) : SuchThatClause(firstArg, secondArg) {
    ...
    if (!argIsValid(firstArg) || !argIsValid(secondArg)) {
        if (!pql::SyntaxCheck::isSyntaxCheck()) throw SemanticError("Follows Star Clause: Argument cannot be a variable, constant or procedure");
    }
}
...
bool FollowsStarClause::argIsValid(QueryArg arg) {
