...
FollowsStarClause::FollowsStarClause(QueryArg firstArg, QueryArg secondArg) : SuchThatClause(firstArg, secondArg) {
    if (!SyntaxCheck::isStmtRef(firstArg) || !SyntaxCheck::isStmtRef(secondArg)) {
        throw "Follows* Clause: arguments do not match the grammar.";
    }
    if ((firstArg.queryDesignEntity != nullptr &&
        ...
        if (!pql::SyntaxCheck::isSyntaxCheck()) throw SemanticError("Follows Star Clause: First argument cannot be a variable, constant or procedure");
    }
    ...
    if ((secondArg.queryDesignEntity != nullptr &&
        ...
        if (!pql::SyntaxCheck::isSyntaxCheck()) throw SemanticError("Follows Star Clause: Second argument cannot be a variable, constant or procedure");
    }
    if (firstArg.queryDesignEntity != nullptr) {
...
FilterResult FollowsStarClause::executePKBAbsQuery(PkbAbstractor *pkbAbstractor) {
    ...
    if (firstArg.argValue != nullptr && secondArg.argValue != nullptr) {
        ...
        pkbResults = pkbAbstractor->getFollowsStar(stmtNum, stmtNum1);
    } else if (firstArg.argValue != nullptr && secondArg.isWildCard) {
        // NumWildcard
        stmtNum = std::stoi(firstArg.argValue->value);
        designEntity1 = DesignEntity::NONE;
        pkbResults = pkbAbstractor->getFollowsStar(stmtNum, designEntity1);
        ...
        stmtNum1 = std::stoi(secondArg.argValue->value);
        pkbResults = pkbAbstractor->getFollowsStar(designEntity, stmtNum1);
    } else if (firstArg.isWildCard && secondArg.isWildCard) {
        // WildcardWildcard
        designEntity = DesignEntity::NONE;
        designEntity1 = DesignEntity::NONE;
        pkbResults = pkbAbstractor->getFollowsStar(designEntity, designEntity1);
