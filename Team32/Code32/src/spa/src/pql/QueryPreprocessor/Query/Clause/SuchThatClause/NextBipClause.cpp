...
NextBipClause::NextBipClause(QueryArg firstArg, QueryArg secondArg) : SuchThatClause(firstArg, secondArg) {
    ...
    if (!argIsValid(firstArg) || !argIsValid(secondArg)) {
        if (!pql::SyntaxCheck::isSyntaxCheck()) throw SemanticError("Next Bip Clause: Argument cannot be a variable, constant or procedure");
    }
}
...
bool NextBipClause::argIsValid(QueryArg arg) {
