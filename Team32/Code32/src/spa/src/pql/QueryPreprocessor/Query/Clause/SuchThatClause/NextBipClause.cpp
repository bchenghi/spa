...
using pql::NextBipClause;

NextBipClause::NextBipClause(QueryArg firstArg, QueryArg secondArg) : SuchThatClause(firstArg, secondArg) {
    if (!SyntaxCheck::isStmtRef(firstArg) || !SyntaxCheck::isStmtRef(secondArg)) {
        throw "Next Bip Clause: arguments do not match the grammar.";
    }
    ...
    firstArg.argValue->designEntity == DesignEntity::PROCEDURE))) {
        if (!pql::SyntaxCheck::isSyntaxCheck()) throw SemanticError("Next Bip Clause: First argument cannot be a variable, constant or procedure");
    }
    ...
    secondArg.argValue->designEntity == DesignEntity::PROCEDURE))) {
        if (!pql::SyntaxCheck::isSyntaxCheck()) throw SemanticError("Next Bip Clause: Second argument cannot be a variable, constant or procedure");
    }
...
FilterResult NextBipClause::executePKBAbsQuery(PkbAbstractor *pkbAbstractor) {
    int stmtNum;
    ...
    if (firstArg.argValue != nullptr && secondArg.argValue != nullptr) {
        ...
        pkbResults = pkbAbstractor->getNextBip(stmtNum, stmtNum1);
    } else if (firstArg.argValue != nullptr && secondArg.isWildCard) {
        // NumWildcard
        stmtNum = std::stoi(firstArg.argValue->value);
        designEntity1 = DesignEntity::NONE;
        pkbResults = pkbAbstractor->getNextBip(stmtNum, designEntity1);
        ...
        stmtNum1 = std::stoi(secondArg.argValue->value);
        pkbResults = pkbAbstractor->getNextBip(designEntity, stmtNum1);
    } else if (firstArg.isWildCard && secondArg.isWildCard) {
        // WildcardWildcard
        designEntity = DesignEntity::NONE;
        designEntity1 = DesignEntity::NONE;
        pkbResults = pkbAbstractor->getNextBip(designEntity, designEntity1);
